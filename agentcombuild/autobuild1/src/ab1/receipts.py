...
OUTSIDE_BODY = ("id", "signer", "signature")
"""signer/signature ride OUTSIDE the hashed body (qp rule): later attempts
may attach authority envelopes without breaking the content id."""
...
def verify(receipt):
    ...
    if not isinstance(receipt, dict) or "id" not in receipt:
        return False
    body = {k: v for k, v in receipt.items() if k not in OUTSIDE_BODY}
