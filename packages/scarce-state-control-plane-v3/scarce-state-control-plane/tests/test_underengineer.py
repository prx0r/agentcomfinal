...
class T(unittest.TestCase):
 ...
 def test_asset_campaign_first_event_is_asset_event(self):
  ...
  for p in projects():
   if p['strategic_role']=='asset_campaign': self.assertIn(p['underengineer']['first_real_moat_event'],{a['acquisition_event'] for a in p['scarce_assets']})
 def test_order_pulls_only_explicit_infra(self):
  o=order('breadup'); self.assertEqual(o['primitive'],'UNDERENGINEER/2.0'); self.assertIn('qp',o['required_components_now']); self.assertNotIn('plugin',o['required_components_now']); self.assertIsNotNone(o['recommended_external_world'])
 def test_feature_default_not_now(self):
  self.assertEqual(feature_allowed('pogtown','would_be_cool')['decision'],'NOT_NOW'); self.assertEqual(feature_allowed('pogtown','verification')['decision'],'NOW')
